...
export default function EmployeePayrollPage() {
  ...
  return (
    <div className="space-y-6 text-left font-sans w-full max-w-7xl mx-auto">
      <div className="bg-gradient-primary text-white p-6 rounded-2xl shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/80 text-xs mt-0.5">{user?.role} • {user?.department || 'Engineering'}</p>
        </div>
        <div className="bg-white/20 backdrop-blur-md px-4 py-3 rounded-xl text-right">
          <span className="text-[10px] uppercase font-bold text-white/80 block">Current Net Salary</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4">
            <h3 className="text-base font-bold text-foreground flex items-center gap-2">
              <DollarSign className="w-5 h-5 text-primary" /> Itemized Compensation Breakdown
            </h3>
            <div className="space-y-2 text-xs divide-y divide-border/60">
              ...
              <div className="pt-3 flex justify-between text-sm font-extrabold text-primary"><span>Monthly Net Pay</span><span>{formatINR(structure?.netSalary)}</span></div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm lg:col-span-2 space-y-4">
            <h3 className="text-base font-bold text-foreground flex items-center gap-2">
              <FileCode className="w-5 h-5 text-primary" /> Published Monthly Payslips
            </h3>
                  ...
                  <div key={ps.id} className="flex items-center justify-between p-4 border border-border/60 rounded-xl hover:bg-muted/30 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="font-extrabold text-primary text-sm">{formatINR(ps.netSalary)}</p>
                        <p className="text-[10px] text-muted-foreground">Gross: {formatINR(ps.grossSalary)}</p>
                      </div>
                      <button
                        onClick={() => handleViewPayslip(ps.id)}
                        className="px-3 py-1.5 btn-primary rounded-lg font-bold text-xs flex items-center gap-1 shadow-sm"
                      >
